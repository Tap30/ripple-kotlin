...
class EventSpecsTest {
    ...
    fun `events namespace supports all event categories`() {
        ...
        val product = Product(
            price = Money(amount = 0, currency = ""),
            productId = ""
        )
        val order = Order(
            orderId = "",
            products = emptyList(),
            revenue = Money(amount = 0, currency = ""),
            total = Money(amount = 0, currency = "")
        )
        ...
        val methods = listOf<Pair<String, EventsNamespace.() -> Unit>>(
            ...
            "checkout_started" to { checkoutStarted(CheckoutPayload(checkout)) },
            "checkout_step_viewed" to { checkoutStepViewed(CheckoutPayload(checkout)) },
            "checkout_step_completed" to { checkoutStepCompleted(CheckoutPayload(checkout)) },
            "order_completed" to { orderCompleted(OrderPayload(order)) },
            "order_failed" to { orderFailed(OrderPayload(order, reason = "err")) },
            "order_cancelled" to { orderCancelled(OrderPayload(order)) },
            "order_shipped" to { orderShipped(OrderPayload(order)) },
            "order_refunded" to { orderRefunded(OrderPayload(order)) },
            "order_updated" to { orderUpdated(OrderPayload(order)) },
            "order_product_fulfilled" to { orderProductFulfilled(OrderProductPayload(order, product)) },
            "order_product_returned" to {
                orderProductReturned(
                    OrderProductPayload(
                        ...
                        product = product,
                        reason = "r",
                        totalReturnedValue = Money(amount = 0, currency = "")
                    )
                )
            },
            ...
            "coupon_entered" to { couponEntered(CouponCheckoutPayload(coupon, checkout)) },
            "coupon_removed" to { couponRemoved(CouponCheckoutPayload(coupon, checkout)) },
            "coupon_denied" to { couponDenied(CouponCheckoutPayload(coupon, checkout, reason = "r")) },
            "coupon_redeemed" to { couponRedeemed(CouponOrderPayload(coupon, order)) },
            ...
            "payment_authorized" to { paymentAuthorized(PaymentPayload(payment)) },
            "payment_captured" to { paymentCaptured(PaymentPayload(payment)) },
            "payment_failed" to { paymentFailed(PaymentPayload(payment)) },
            "payment_refunded" to {
                paymentRefunded(
                    PaymentPayload(
                        payment = payment,
                        returnedAmount = Money(amount = 0, currency = "")
                    )
                )
            },
            "referral_shared" to { referralShared(ReferralPayload(referral)) },
            "referral_applied" to { referralApplied(ReferralPayload(referral)) },
            "incentive_granted" to { incentiveGranted(IncentivePayload(incentive)) },
            "incentive_redeemed" to { incentiveRedeemed(IncentivePayload(incentive)) },
            "incentive_claimed" to { incentiveClaimed(IncentivePayload(incentive)) },
            "incentive_expired" to { incentiveExpired(IncentivePayload(incentive)) },
            "challenge_started" to { challengeStarted(ChallengePayload(challenge)) },
            "challenge_completed" to { challengeCompleted(ChallengePayload(challenge)) },
            "challenge_step_completed" to { challengeStepCompleted(ChallengePayload(challenge, step = "0")) }
        )
    ...
    fun `predefined event preserves nested money category and custom properties`() {
        val product = Product(
            productId = "p1",
            productTitle = "Coffee",
            price = Money(amount = 12_500, currency = "IRR"),
        ...
        assertEquals(12_500, productJson["price"]?.jsonObject?.get("amount")?.jsonPrimitive?.int)
    ...
    fun `cart events encode cart object and custom properties`() {
        val product = Product(
            productId = "p1",
            price = Money(amount = 10, currency = "IRR")
        )
        val cart = Cart(
            cartId = "cart-1",
    ...
    fun `address and coupon preserve custom properties`() {
        ...
        val coupon = Coupon(
            code = "SAVE",
            amount = Money(amount = 1000, currency = "IRR"),
            customProperties = buildJsonObject { put("campaign", "summer") }
        )

        val shippingPayload = Shipping(
            price = Money(amount = 500, currency = "IRR"),
            method = "standard",
            destination = address
        ).toJsonPayload()
        val couponPayload = CouponCheckoutPayload(
            coupon = coupon,
            checkout = Checkout(
                order = Order(
                    orderId = "o1",
                    products = emptyList(),
                    revenue = Money(amount = 0, currency = "IRR"),
                    total = Money(amount = 0, currency = "IRR")
                ),
                step = "1"
            )
        ).toJsonPayload()
